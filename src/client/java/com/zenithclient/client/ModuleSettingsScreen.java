...
public final class ModuleSettingsScreen extends Screen {
    ...
    private List<String[]> optionRows() {
        ...
        switch (type) {
            ...
            case XRAY -> { rows.add(row("Visible blocks", selectionSummary(config.xraySearch, config.xrayMode.displayName()))); rows.add(row("Renderer", "Chunk rebuild mode")); rows.add(row("Target list", selectionSummary(config.xraySearch, "Using X-Ray mode"))); }
