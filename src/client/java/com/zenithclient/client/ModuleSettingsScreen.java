...
public final class ModuleSettingsScreen extends Screen {
    ...
    private Numeric numeric(int index) {
        return switch (type) {
            ...
            case MACE_KILL -> switch (index) { case 1 -> new Numeric(4.0, 6.0, 0.1, false); default -> null; };
    ...
    private List<String[]> optionRows() {
        ...
        switch (type) {
            ...
            case BLOCKS -> { rows.add(row("Blocks", selectionSummary(config.blockSearch, config.blockHighlightMode.displayName()))); rows.add(row("Search radius", config.blockRadius + " blocks")); rows.add(row("Outline color", colorName(config.blockOutlineColor))); rows.add(row("Fill opacity", config.blockFillOpacity + "%")); rows.add(row("Target list", selectionSummary(config.blockSearch, "Using block mode"))); }
            ...
            case XRAY -> { rows.add(row("Visible blocks", selectionSummary(config.xraySearch, config.xrayMode.displayName()))); rows.add(row("Ore lighting", "MAXIMUM")); rows.add(row("Target list", selectionSummary(config.xraySearch, "Using X-Ray mode"))); }
            ...
            case KILL_AURA -> { rows.add(row("Kill Aura", onOff(config.killAura))); rows.add(row("Range", trim(config.killAuraRange))); rows.add(row("Reach", trim(config.reachDistance))); rows.add(row("Entities", selectionSummary(config.killAuraSearch, "All configured entities"))); }
    ...
    private boolean isEntityPickerRow(int index) {
        return (type == Type.ENTITY && index == 0)
                || (type == Type.KILL_AURA && index == 3)
                || (type == Type.BLOCKS && (index == 0 || index == 4))
                || (type == Type.XRAY && (index == 0 || index == 2));
    }
    ...
    private String description() { return switch (type) { case PLAYER -> "Highlights other players with a stable model outline."; case ENTITY -> "Choose one or more ESP targets from the entity list."; case ITEM -> "Highlights dropped items."; case PROJECTILE -> "Highlights arrows and other projectiles."; case BLOCKS -> "Shows selected nearby blocks with an overlay."; case TRAJECTORY -> "Predicts projectile paths and collision targets."; case XRAY -> "Shows selected blocks with one controlled renderer rebuild."; case NO_BLINDNESS -> "Removes blindness and darkness effects client-side."; case NO_FIRE_OVERLAY -> "Hides the first-person burning overlay."; case FLIGHT -> "Controls horizontal, vertical, and sprint flight speed."; case SPEED -> "Applies custom ground movement speed."; case AUTO_TOTEM -> "Refills your offhand from inventory."; case ATTRIBUTE_SWAP -> "Swaps to a hotbar slot for the attack, then restores it."; case KILL_AURA -> "Attacks nearby targets from a configurable entity list."; case REACH -> "Extends attack targeting range."; case INFINITE_REACH -> "Uses the maximum configured client-side reach."; case MACE_KILL -> "Uses a small bounded simulated fall instead of extreme heights."; case FREECAM -> "Detached camera with normal WASD directions and smooth movement."; default -> "Click the keybind row to bind a key."; }; }
    ...
    private static String selectionSummary(String value, String emptyLabel) {
        if (empty(value)) return emptyLabel + " • click to edit";
        int count = 0;
        for (String token : value.split(",")) if (!token.isBlank()) count++;
        return count == 1 ? "1 selected • click to edit" : count + " selected • click to edit";
    }
